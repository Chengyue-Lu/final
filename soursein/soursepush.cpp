...
void soursepush::soursew25q64(int length, char data[][3], uint32_t address)
{
    ...
    if (3 * length > 256)
    {
        W25Q64.PageWrite(address, (uint8_t *)sequantial_data, 256);
        W25Q64.PageWrite(address + 256 * 16, (uint8_t *)sequantial_data + 256, 3 * length - 256);
    }
    else
    {
...
void soursepush::sourseread(int length, char data[][3], uint32_t address)
{
    ...
    if (3 * length > 256)
    {
        W25Q64.ReadData(address, data_rec, 256);
        W25Q64.ReadData(address + 256 * 16, data_rec + 256, 3 * length - 256);
    }
    else
    {
